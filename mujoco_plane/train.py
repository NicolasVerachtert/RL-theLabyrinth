...
def train(model_name: str):
    ...
    # TOTAL_STEPS = 1_000_000 # Define how many total steps the agent should train for (across all episodes and environments)
    # RENDER_MODE = "rgb_array" # Set the rendering mode (useful for visual debugging, "rgb_array" enables rendering for logs)
    # N_ENVS = 6 # Number of parallel environments for training (vectorized environments speed up learning by running multiple instances in parallel)

    EPISODE_LENGTH = 100 # Define the length of each episode (i.e., max number of steps in one episode)
    TOTAL_STEPS = 1_000 # Define how many total steps the agent should train for (across all episodes and environments)
    RENDER_MODE = "rgb_array" # Set the rendering mode (useful for visual debugging, "rgb_array" enables rendering for logs)
    N_ENVS = 1 # Number of parallel environments for training (vectorized environments speed up learning by running multiple instances in parallel)
    ...
    model.learn(
        ...
        log_interval=20,  # Log training progress every 20 episodes
